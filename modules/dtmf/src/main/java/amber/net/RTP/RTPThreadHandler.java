...
public class RTPThreadHandler extends Thread
{
   ...
   */
   ...
   /**
   *    Starts the RTP Receiver. This method instantiates a multicast socket on the 
   *    multicast address and port specified and listens for packets on that multicast 
   *    group. When it receives a packet, it parses the packet out and updates several
   *    session and source level statistics. It also posts an event to the Application 
   *    about the reception of an RTP Packet
   */
...
/*    		s.leaveGroup( m_InetAddress );
  ...
  */
   ...
   public int sendTestPacket( String testString )
   {
        byte AppPkt[] = testString.getBytes();
